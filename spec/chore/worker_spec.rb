...
describe Chore::Worker do
  ...
  shared_examples_for "a worker" do
    it 'processing a single job' do
      ...
      expect(SimpleJob).to receive(:perform).with(*payload)
      expect(consumer).to receive(:complete).with('1', nil)
    ...
    it 'processing multiple jobs' do
      ...
      expect(SimpleJob).to receive(:perform).exactly(10).times
      expect(consumer).to receive(:complete).exactly(10).times
  ...
  describe 'expired?' do
    ...
    it 'should not be expired when before total timeout' do
      expect(worker).not_to be_expired
    end
  ...
  describe 'with errors' do
    context 'on parse' do
      ...
      it 'should fail cleanly' do
        ...
        expect(consumer).not_to receive(:complete)
        expect(Chore).to receive(:run_hooks_for).with(:on_failure, job, anything())
        Chore::Worker.start(work)
      end

      it 'should reject job' do
        work = Chore::UnitOfWork.new(2,nil,'test',60,job,0,consumer)
        expect(consumer).to receive(:reject).with(2)
        Chore::Worker.start(work)
      end

      context 'more than the maximum allowed times' do
        before(:each) do
          allow(Chore.config).to receive(:max_attempts).and_return(10)
        end

        it 'should permanently fail' do
          work = Chore::UnitOfWork.new(2,nil,'test',60,job,9,consumer)
          expect(Chore).to receive(:run_hooks_for).with(:on_permanent_failure, 'test', job, anything())
          Chore::Worker.start(work)
        end

        it 'should mark the item as completed' do
          work = Chore::UnitOfWork.new(2,nil,'test',60,job,9,consumer)
          expect(consumer).to receive(:complete).with(2, nil)
          Chore::Worker.start(work)
        end
      end
    end
    ...
    context 'on perform' do
      ...
      before(:each) do
        allow(SimpleJob).to receive(:perform).and_raise(ArgumentError)
      ...
      context 'more than the maximum allowed times' do
        it 'should permanently fail' do
          ...
          expect(SimpleJob).to receive(:run_hooks_for).with(:on_permanent_failure, 'test', parsed_job, anything())
          Chore::Worker.start(work)
        end
