...
describe Chore::Tapjoy::Monitoring do
  ...
  it 'should process after message hooks with success or failure' do
    work = []
    work << Chore::UnitOfWork.new('1', 'test', Chore::JsonEncoder.encode(job), 0, consumer)
    work << Chore::UnitOfWork.new('2', 'test', Chore::JsonEncoder.encode(breaking_job), 0, consumer)
    work << Chore::UnitOfWork.new('4', 'test', Chore::JsonEncoder.encode(rejected_job), 0, consumer)
    consumer.should_receive(:complete).with('1')
